...
/** Loads the active image projection and the staff-only removed-image audit view. */
export function TicketImages({ ticketId }: Readonly<{ ticketId: string }>) {
  const { user } = useAuth()
...
function TicketImageCollections({
  canViewRemoved,
  images,
}: Readonly<{
    ...
    <div className="space-y-7">
      <div className="space-y-3">
        ...
        <MediaGallery
          ...
          renderActions={(asset) => (
            <TicketImageMetadata
              image={activeImages.find((image) => image.asset.id === asset.id)}
            />
          )}
        />
      </div>
      ...
      {canViewRemoved ? (
        <div className="border-outline-variant space-y-3 border-t pt-6">
          ...
          <MediaGallery
            ...
            renderActions={(asset) => (
              <TicketImageMetadata
                image={removedImages.find((image) => image.asset.id === asset.id)}
              />
            )}
          />
        </div>
