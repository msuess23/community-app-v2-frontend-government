...
import { executeApiRequest } from '@/api/client/api-request'
...
describe('executeApiRequest', () => {
  ...
  it('returns a JSON response and uses the resolved API URL', async () => {
    ...
    await expect(
      executeApiRequest<{ id: string }>('/users/me'),
    ).resolves.toEqual({
      id: 'user-1',
    })
  ...
  it('returns undefined for a no-content response', async () => {
    ...
    await expect(
      executeApiRequest<void>('/auth/logout'),
  ...
  it('returns binary responses as blobs', async () => {
    ...
    const result = await executeApiRequest<Blob>('/documents/example', {
      responseType: 'blob',
    })
  ...
  it('passes an abort signal to fetch', async () => {
    ...
    await executeApiRequest('/health', { signal: controller.signal })
  ...
  it('normalizes the project error envelope', async () => {
    ...
    const error = await captureApiError(() =>
      executeApiRequest('/auth/register'),
    )
  ...
  it('normalizes native FastAPI validation details', async () => {
    ...
    const error = await captureApiError(() => executeApiRequest('/users'))
  ...
  it('wraps network failures but preserves abort errors', async () => {
    ...
    await expect(executeApiRequest('/health')).rejects.toMatchObject({
    ...
    await expect(executeApiRequest('/health')).rejects.toBe(abortError)
  })
})
